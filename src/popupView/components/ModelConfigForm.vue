...
async function handleTest() {
  ...
  try {
    ...
    const client = new ChatClient(testConfig);
  ...
  <div class="block font-[inherit] space-y-4">
    ...
    <div class="pt-3 border-t border-slate-100">
      <button
        ...
        class="w-full h-9 text-sm font-medium text-sky-600 bg-sky-50 border border-sky-200 rounded-lg transition-all duration-150 hover:bg-sky-100 active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed"
      >
        <span v-if="testing">测试中...</span>
        <span v-else>测试模型连接</span>
      </button>
      <p
        v-if="testResult"
        class="mt-2 text-xs leading-relaxed"
        :class="testResult.ok ? 'text-emerald-600' : 'text-red-500'"
      >
